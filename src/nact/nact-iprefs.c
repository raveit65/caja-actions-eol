 ...
 *
 * Defaults to exporting as a MateConfEntry (see. #nact-iprefs.h)
 *
...
nact_iprefs_migrate_key( const BaseWindow *window, const gchar *old_key, const gchar *new_key )
{
	...
	if( st_initialized && !st_finalized ){
		...
		mateconf_client = NACT_IPREFS_GET_INTERFACE( window )->private->client;
...
nact_iprefs_write_bool( const BaseWindow *window, const gchar *name, gboolean value )
{
	...
	if( st_initialized && !st_finalized ){
		...
		na_mateconf_utils_write_bool( NACT_IPREFS_GET_INTERFACE( window )->private->client, path, value, NULL );
		g_free( path );
	}
}
...
nact_iprefs_write_string( const BaseWindow *window, const gchar *name, const gchar *value )
{
	...
	if( st_initialized && !st_finalized ){
		...
		na_mateconf_utils_write_string( NACT_IPREFS_GET_INTERFACE( window )->private->client, path, value, NULL );
