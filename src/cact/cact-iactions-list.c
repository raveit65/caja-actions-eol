 ...
 */
struct CactIActionsListInterfacePrivate {
...
static void     interface_base_init( CactIActionsListInterface *klass );
static void     interface_base_finalize( CactIActionsListInterface *klass );

static void     free_items_callback( CactIActionsList *instance, GList *items );
static void     free_column_edited_callback( CactIActionsList *instance, NAObject *object, gchar *text, gint column );

static gboolean are_profiles_displayed( CactIActionsList *instance, IActionsListInstanceData *ialid );
static void     display_label( GtkTreeViewColumn *column, GtkCellRenderer *cell, GtkTreeModel *model, GtkTreeIter *iter, CactIActionsList *instance );
static gboolean filter_selection( GtkTreeSelection *selection, GtkTreeModel *model, GtkTreePath *path, gboolean path_currently_selected, CactIActionsList *instance );
...
static gboolean have_dnd_mode( CactIActionsList *instance, IActionsListInstanceData *ialid );
static gboolean have_filter_selection_mode( CactIActionsList *instance, IActionsListInstanceData *ialid );
static void     inline_edition( CactIActionsList *instance );
static gboolean is_iduplicable_proxy( CactIActionsList *instance, IActionsListInstanceData *ialid );
static gboolean on_button_press_event( GtkWidget *widget, GdkEventButton *event, CactIActionsList *instance );
static void     on_edition_status_changed( CactIActionsList *instance, NAIDuplicable *object );
static gboolean on_focus_in( GtkWidget *widget, GdkEventFocus *event, CactIActionsList *instance );
static gboolean on_focus_out( GtkWidget *widget, GdkEventFocus *event, CactIActionsList *instance );
static gboolean on_key_pressed_event( GtkWidget *widget, GdkEventKey *event, CactIActionsList *instance );
static void     on_label_edited( GtkCellRendererText *renderer, const gchar *path, const gchar *text, CactIActionsList *instance );
static void     on_tab_updatable_item_updated( CactIActionsList *instance, NAObject *object, gboolean force_display );
static void     open_popup( CactIActionsList *instance, GdkEventButton *event );

GType
cact_iactions_list_get_type( void )
{
...
register_type( void )
{
	static const gchar *thisfn = "cact_iactions_list_register_type";
	GType type;

	static const GTypeInfo info = {
		sizeof( CactIActionsListInterface ),
	...
	type = g_type_register_static( G_TYPE_INTERFACE, "CactIActionsList", &info, 0 );
...
static void
interface_base_init( CactIActionsListInterface *klass )
{
	static const gchar *thisfn = "cact_iactions_list_interface_base_init";

	if( !st_iactions_list_initialized ){

		g_debug( "%s: klass=%p", thisfn, ( void * ) klass );

		klass->private = g_new0( CactIActionsListInterfacePrivate, 1 );

		/**
		 * cact-iactions-list-count-updated:
		 *
		...
		st_signals[ LIST_COUNT_UPDATED ] = g_signal_new(
				...
				NULL,
				NULL,
				cact_marshal_VOID__INT_INT_INT,
				G_TYPE_NONE,
				3,
				G_TYPE_INT,
				G_TYPE_INT,
		...
		/**
		 * cact-iactions-list-selection-changed:
		 *
		 ...
		 * objects as user_data (see #cact_iactions_list_on_treeview_selection_changed()).
		 *
		...
		/**
		 * cact-iactions-list-column-edited:
		 *
		...
		st_signals[ COLUMN_EDITED ] = g_signal_new_class_handler(
				...
				NULL,
				NULL,
				cact_marshal_VOID__POINTER_POINTER_INT,
				G_TYPE_NONE,
				3,
				G_TYPE_POINTER,
				G_TYPE_POINTER,
		...
		/**
		 * cact-iactions-list-status-changed:
		 *
		 * This signal is emitted byIActionsList to its implementor
...
static void
free_items_callback( CactIActionsList *instance, GList *items )
{
	g_debug( "cact_iactions_list_free_items_callback: selection=%p (%d items)",
...
static void
free_column_edited_callback( CactIActionsList *instance, NAObject *object, gchar *text, gint column )
{
	static const gchar *thisfn = "cact_iactions_list_free_column_edited_callback";
...
static void
interface_base_finalize( CactIActionsListInterface *klass )
{
	static const gchar *thisfn = "cact_iactions_list_interface_base_finalize";
...
/**
 * cact_iactions_list_initial_load_toplevel:
 * @instance: this #CactIActionsList *instance.
 *
 * Allocates and initializes the ActionsList widget.
 *
 * GtkTreeView is created with CactTreeModel model
 * CactTreeModel
 ...
 */
void
cact_iactions_list_initial_load_toplevel( CactIActionsList *instance )
{
	static const gchar *thisfn = "cact_iactions_list_initial_load_toplevel";
	GtkWidget *label;
	...
	if( st_iactions_list_initialized && !st_iactions_list_finalized ){
		...
		cact_tree_model_initial_load( BASE_WINDOW( instance ), treeview );
...
/**
 * cact_iactions_list_runtime_init_toplevel:
 ...
 */
void
cact_iactions_list_runtime_init_toplevel( CactIActionsList *instance, GList *items )
{
	static const gchar *thisfn = "cact_iactions_list_runtime_init_toplevel";
	GtkTreeView *treeview;
	CactTreeModel *model;
	gboolean have_dnd;
	...
	if( st_iactions_list_initialized && !st_iactions_list_finalized ){
		...
		cact_tree_model_runtime_init( model, have_dnd );
		...
		base_window_signal_connect(
				...
				"changed",
				G_CALLBACK( cact_iactions_list_on_treeview_selection_changed ));
		...
		/* records CactIActionsList as a proxy for edition status
		 * modification */
		 ...
		 */
		cact_iactions_list_fill( instance, items );
...
/**
 * cact_iactions_list_all_widgets_showed:
 * @window: this #CactIActionsList *instance.
 */
void
cact_iactions_list_all_widgets_showed( CactIActionsList *instance )
{
	static const gchar *thisfn = "cact_iactions_list_all_widgets_showed";
	...
	if( st_iactions_list_initialized && !st_iactions_list_finalized ){
		...
		cact_iactions_list_bis_select_first_row( instance );
	}
}

/**
 * cact_iactions_list_dispose:
 * @window: this #CactIActionsList *instance.
 */
void
cact_iactions_list_dispose( CactIActionsList *instance )
{
	static const gchar *thisfn = "cact_iactions_list_dispose";
	GtkTreeView *treeview;
	CactTreeModel *model;
	IActionsListInstanceData *ialid;
...
/**
 * cact_iactions_list_brief_tree_dump:
 * @instance: this #CactIActionsList implementation.
 *
 * Brief dump of the tree store content.
 */
void
cact_iactions_list_brief_tree_dump( CactIActionsList *instance )
{
	GtkTreeView *treeview;
	CactTreeModel *model;
...
/**
 * cact_iactions_list_collapse_all:
 * @instance: this #CactIActionsList implementation.
 *
 * Collapse all the tree hierarchy.
 */
void
cact_iactions_list_collapse_all( CactIActionsList *instance )
{
	static const gchar *thisfn = "cact_iactions_list_collapse_all";
	GtkTreeView *treeview;
...
/**
 * cact_iactions_list_display_order_change:
 * @instance: this #CactIActionsList implementation.
 ...
 */
void
cact_iactions_list_display_order_change( CactIActionsList *instance, gint order_mode )
{
	GtkTreeView *treeview;
	CactTreeModel *model;
...
/**
 * cact_iactions_list_expand_all:
 * @instance: this #CactIActionsList implementation.
 *
 * Expand all the tree hierarchy.
 */
void
cact_iactions_list_expand_all( CactIActionsList *instance )
{
	static const gchar *thisfn = "cact_iactions_list_expand_all";
	GtkTreeView *treeview;
 ...
 */
void
cact_iactions_list_fill( CactIActionsList *instance, GList *items )
{
	static const gchar *thisfn = "cact_iactions_list_fill";
	GtkTreeView *treeview;
	CactTreeModel *model;
	...
	if( st_iactions_list_initialized && !st_iactions_list_finalized ){
		...
		cact_iactions_list_bis_clear_selection( instance, treeview );

		ialid = cact_iactions_list_priv_get_instance_data( instance );
		profiles_are_displayed = are_profiles_displayed( instance, ialid );

		ialid->selection_changed_allowed = FALSE;
		cact_tree_model_fill( model, items, profiles_are_displayed );
...
/**
 * cact_iactions_list_get_management_mode:
 * @instance: this #CactIActionsList instance.
 *
 * Returns: the current management mode of the list.
 */
gint
cact_iactions_list_get_management_mode( CactIActionsList *instance )
{
	gint mode = 0;
	IActionsListInstanceData *ialid;

	g_return_val_if_fail( CACT_IS_IACTIONS_LIST( instance ), 0 );
...
/**
 * cact_iactions_list_has_modified_items:
 * @window: this #CactIActionsList instance.
 *
 * Returns: %TRUE if at least there is one modified item in the list.
 */
gboolean
cact_iactions_list_has_modified_items( CactIActionsList *instance )
{
	...
	g_return_val_if_fail( CACT_IS_IACTIONS_LIST( instance ), FALSE );
	...
	if( st_iactions_list_initialized && !st_iactions_list_finalized ){
		...
		model = CACT_TREE_MODEL( gtk_tree_view_get_model( treeview ));
		cact_tree_model_iter( model, ( FnIterOnStore ) has_modified_iter, &has_modified );*/
	}
...
/**
 * cact_iactions_list_on_treeview_selection_changed:
 ...
 */
void
cact_iactions_list_on_treeview_selection_changed( GtkTreeSelection *selection, CactIActionsList *instance )
{
	...
	if( ialid->selection_changed_allowed ){
		...
		g_debug( "cact_iactions_list_on_treeview_selection_changed" );
		g_signal_handler_block( instance, ialid->tab_updated_handler );

		selected_items = cact_iactions_list_bis_get_selected_items( instance );
		g_debug( "cact_iactions_list_on_treeview_selection_changed: selection=%p (%d items)", ( void * ) selected_items, g_list_length( selected_items ));
 ...
 */
GList *
cact_iactions_list_remove_rec( GList *list, NAObject *object )
{
	GList *subitems, *it;
	...
	if( NA_IS_OBJECT_ITEM( object )){
		...
		for( it = subitems ; it ; it = it->next ){
			list = cact_iactions_list_remove_rec( list, it->data );
		}
	}
...
/**
 * cact_iactions_list_set_management_mode:
 * @instance: this #CactIActionsList instance.
 ...
 */
void
cact_iactions_list_set_management_mode( CactIActionsList *instance, gint mode )
{
	GtkTreeView *treeview;
	GtkTreeSelection *selection;
	gboolean multiple;
...
static gboolean
are_profiles_displayed( CactIActionsList *instance, IActionsListInstanceData *ialid )
{
	gboolean display;
 ...
 */
static void
display_label( GtkTreeViewColumn *column, GtkCellRenderer *cell, GtkTreeModel *model, GtkTreeIter *iter, CactIActionsList *instance )
{
	NAObject *object;
	gchar *label;
	...
	/*g_debug( "cact_iactions_list_display_label: %s %s", G_OBJECT_TYPE_NAME( object ), label );*/
	...
	if( ialid->management_mode == IACTIONS_LIST_MANAGEMENT_MODE_EDITION ){
		...
		writable_item = cact_window_is_item_writable( CACT_WINDOW( instance ), item, NULL );
 ...
 */
static gboolean
filter_selection( GtkTreeSelection *selection, GtkTreeModel *model, GtkTreePath *path, gboolean path_currently_selected, CactIActionsList *instance )
{
	static const gchar *thisfn = "cact_iactions_list_filter_selection";
...
filter_selection_is_implicitely_selected( NAObject *object )
{
	...
	selected = ( gboolean ) GPOINTER_TO_UINT( g_object_get_data( G_OBJECT( object), "cact-implicit-selection" ));
...
filter_selection_set_implicitely_selected_childs( NAObject *object, gboolean select )
{
	...
	if( NA_IS_OBJECT_ITEM( object )){
		...
		for( ic = childs ; ic ; ic = ic->next ){
			g_object_set_data( G_OBJECT( ic->data ), "cact-implicit-selection", GUINT_TO_POINTER(( guint ) select ));
...
static gboolean
have_dnd_mode( CactIActionsList *instance, IActionsListInstanceData *ialid )
{
	gboolean have_dnd;
...
static gboolean
have_filter_selection_mode( CactIActionsList *instance, IActionsListInstanceData *ialid )
{
 ...
 */
static void
inline_edition( CactIActionsList *instance )
{
	static const gchar *thisfn = "cact_iactions_list_inline_edition";
	IActionsListInstanceData *ialid;
	GtkTreeView *treeview;
	GtkTreeSelection *selection;
	GList *listrows;
...
static gboolean
is_iduplicable_proxy( CactIActionsList *instance, IActionsListInstanceData *ialid )
{
	gboolean is_proxy;

	is_proxy = ( ialid->management_mode == IACTIONS_LIST_MANAGEMENT_MODE_EDITION );
	g_debug( "cact_iactions_list_is_iduplicable_proxy: is_proxy=%s", is_proxy ? "True":"False" );

	return( is_proxy );
}

static gboolean
on_button_press_event( GtkWidget *widget, GdkEventButton *event, CactIActionsList *instance )
{
	/*static const gchar *thisfn = "cact_iactions_list_v_on_button_pres_event";
	...
	if( event->type == GDK_2BUTTON_PRESS && event->button == 1 ){
		cact_iactions_list_bis_toggle_collapse( instance );
		stop = TRUE;
	}
...
static void
on_edition_status_changed( CactIActionsList *instance, NAIDuplicable *object )
{
	GtkTreeView *treeview;
	CactTreeModel *model;
	IActionsListInstanceData *ialid;
	...
	g_debug( "cact_iactions_list_on_edition_status_changed: instance=%p, object=%p (%s)",
			( void * ) instance,
	...
	treeview = cact_iactions_list_priv_get_actions_list_treeview( instance );
	model = CACT_TREE_MODEL( gtk_tree_view_get_model( treeview ));
	cact_tree_model_display( model, NA_OBJECT( object ));
	...
	if( na_object_is_modified( object )){
		...
		ialid->modified_items = cact_iactions_list_remove_rec( ialid->modified_items, NA_OBJECT( object ));
	}
 ...
 */
static gboolean
on_focus_in( GtkWidget *widget, GdkEventFocus *event, CactIActionsList *instance )
{
	/*static const gchar *thisfn = "cact_iactions_list_on_focus_in";*/
	gboolean stop = FALSE;
...
static gboolean
on_focus_out( GtkWidget *widget, GdkEventFocus *event, CactIActionsList *instance )
{
	/*static const gchar *thisfn = "cact_iactions_list_on_focus_out";*/
	gboolean stop = FALSE;
...
static gboolean
on_key_pressed_event( GtkWidget *widget, GdkEventKey *event, CactIActionsList *instance )
{
	/*static const gchar *thisfn = "cact_iactions_list_v_on_key_pressed_event";
	g_debug( "%s: widget=%p, event=%p, user_data=%p", thisfn, widget, event, user_data );*/
	gboolean stop = FALSE;
	...
	if( event->keyval == GDK_Right ){
		cact_iactions_list_bis_expand_to_first_child( instance );
		stop = TRUE;
	}

	if( event->keyval == GDK_Left ){
		cact_iactions_list_bis_collapse_to_parent( instance );
		stop = TRUE;
	}
 ...
 */
static void
on_label_edited( GtkCellRendererText *renderer, const gchar *path_str, const gchar *text, CactIActionsList *instance )
{
	GtkTreeView *treeview;
	CactTreeModel *model;
	NAObject *object;
	...
	object = cact_tree_model_object_at_path( model, path );
 ...
 */
static void
on_tab_updatable_item_updated( CactIActionsList *instance, NAObject *object, gboolean force_display )
{
	static const gchar *thisfn = "cact_iactions_list_on_tab_updatable_item_updated";
	GtkTreeView *treeview;
	GtkTreeModel *model;
...
static void
open_popup( CactIActionsList *instance, GdkEventButton *event )
{
	GtkTreeView *treeview;
	GtkTreeModel *model;
	GtkTreePath *path;
	...
	if( gtk_tree_view_get_path_at_pos( treeview, event->x, event->y, &path, NULL, NULL, NULL )){
		...
		cact_main_menubar_open_popup( CACT_MAIN_WINDOW( instance ), event );
	}
}
