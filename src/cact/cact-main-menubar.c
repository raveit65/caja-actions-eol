...
#include "cact-main-menubar-view.h"
#include "cact-main-menubar-tools.h"
#include "cact-main-menubar-maintainer.h"
#include "cact-main-menubar-help.h"

#define MENUBAR_PROP_STATUS_CONTEXT			"cact-menubar-status-context"
#define MENUBAR_PROP_MAIN_STATUS_CONTEXT	"cact-menubar-main-status-context"
#define MENUBAR_PROP_ACTIONS_GROUP			"cact-menubar-actions-group"
...
#define MENUBAR_PROP_ITEM_ACTION			"cact-menubar-item-action"
#endif

static void     on_iactions_list_count_updated( CactMainWindow *window, gint menus, gint actions, gint profiles );
static void     on_iactions_list_selection_changed( CactMainWindow *window, GList *selected );
static void     on_iactions_list_focus_in( CactMainWindow *window, gpointer user_data );
static void     on_iactions_list_focus_out( CactMainWindow *window, gpointer user_data );
static void     on_iactions_list_status_changed( CactMainWindow *window, gpointer user_data );
static void     on_level_zero_order_changed( CactMainWindow *window, gpointer user_data );
static void     on_update_sensitivities( CactMainWindow *window, gpointer user_data );
...
static void     on_menu_item_selected( GtkMenuItem *proxy, CactMainWindow *window );
static void     on_menu_item_deselected( GtkMenuItem *proxy, CactMainWindow *window );
static void     on_popup_selection_done(GtkMenuShell *menushell, CactMainWindow *window );
static void     on_proxy_connect( GtkActionGroup *action_group, GtkAction *action, GtkWidget *proxy, CactMainWindow *window );
static void     on_proxy_disconnect( GtkActionGroup *action_group, GtkAction *action, GtkWidget *proxy, CactMainWindow *window );
...
static const GtkActionEntry entries[] = {
		...
		{ "ImportItem" , GTK_STOCK_CONVERT, N_( "_Import assistant..." ), "",
				...
				G_CALLBACK( cact_main_menubar_tools_on_import ) },
...
static const GtkToggleActionEntry toolbar_entries[] = {
		...
		{ "ViewFileToolbarItem", NULL, N_( "_File" ), NULL,
				...
				G_CALLBACK( cact_main_menubar_view_on_toolbar_file ), FALSE },
		{ "ViewEditToolbarItem", NULL, N_( "_Edit" ), NULL,
				/* i18n: tooltip displayed in the status bar when selecting the 'View Edit toolbar' item */
				N_( "Display the Edit toolbar" ),
				G_CALLBACK( cact_main_menubar_view_on_toolbar_edit ), FALSE },
		{ "ViewToolsToolbarItem", NULL, N_( "_Tools" ), NULL,
				/* i18n: tooltip displayed in the status bar when selecting 'View Tools toolbar' item */
				N_( "Display the Tools toolbar" ),
				G_CALLBACK( cact_main_menubar_view_on_toolbar_tools ), FALSE },
		{ "ViewHelpToolbarItem", NULL, N_( "_Help" ), NULL,
				/* i18n: tooltip displayed in the status bar when selecting 'View Help toolbar' item */
				N_( "Display the Help toolbar" ),
				G_CALLBACK( cact_main_menubar_view_on_toolbar_help ), FALSE },
};

/**
 * cact_main_menubar_runtime_init:
 * @window: the #CactMainWindow to which the menubar is attached.
 *
 * Creates the menubar.
 ...
 */
void
cact_main_menubar_runtime_init( CactMainWindow *window )
{
	static const gchar *thisfn = "cact_main_menubar_init";
	...
	cact_main_toolbar_init( window, action_group );
}

/**
 * cact_main_menubar_dispose:
 * @window: this #CactMainWindow window.
 *
 * Release internally allocated resources.
 */
void
cact_main_menubar_dispose( CactMainWindow *window )
{
	static const gchar *thisfn = "cact_main_menubar_dispose";
	MenubarIndicatorsStruct *mis;
...
/**
 * cact_main_menubar_is_level_zero_order_changed:
 * @window: the #CactMainWindow main window.
 *
 * Returns: %TRUE if the level zero has changed, %FALSE else.
 */
gboolean
cact_main_menubar_is_level_zero_order_changed( const CactMainWindow *window )
{
	MenubarIndicatorsStruct *mis;
 ...
 */
void
cact_main_menubar_open_popup( CactMainWindow *instance, GdkEventButton *event )
{
	GtkUIManager *ui_manager;
	GtkWidget *menu;
	MenubarIndicatorsStruct *mis;
 ...
 */
static void
on_iactions_list_count_updated( CactMainWindow *window, gint menus, gint actions, gint profiles )
{
	MenubarIndicatorsStruct *mis;
	gchar *status;

	g_debug( "cact_main_menubar_on_iactions_list_count_updated: menus=%u, actions=%u, profiles=%u", menus, actions, profiles );
	g_return_if_fail( CACT_IS_MAIN_WINDOW( window ));
	...
	cact_main_statusbar_display_status( window, MENUBAR_PROP_MAIN_STATUS_CONTEXT, status );
	g_free( status );
 ...
 */
static void
on_iactions_list_selection_changed( CactMainWindow *window, GList *selected )
{
	MenubarIndicatorsStruct *mis;

	g_debug( "cact_main_menubar_on_iactions_list_selection_changed: selected=%p (count=%d)",
	...
	g_debug( "cact_main_menubar_on_iactions_list_selection_changed: menus=%d, actions=%d, profiles=%d",
...
static void
on_iactions_list_focus_in( CactMainWindow *window, gpointer user_data )
{
	MenubarIndicatorsStruct *mis;

	g_debug( "cact_main_menubar_on_iactions_list_focus_in" );
	g_return_if_fail( CACT_IS_MAIN_WINDOW( window ));
...
static void
on_iactions_list_focus_out( CactMainWindow *window, gpointer user_data )
{
	MenubarIndicatorsStruct *mis;

	g_debug( "cact_main_menubar_on_iactions_list_focus_out" );
	g_return_if_fail( CACT_IS_MAIN_WINDOW( window ));
...
{
	g_debug( "cact_main_menubar_on_iactions_list_status_changed" );
	g_return_if_fail( CACT_IS_MAIN_WINDOW( window ));
...
static void
on_level_zero_order_changed( CactMainWindow *window, gpointer user_data )
{
	MenubarIndicatorsStruct *mis;

	g_return_if_fail( CACT_IS_MAIN_WINDOW( window ));

	g_debug( "cact_main_menubar_on_level_zero_order_changed: change=%s", user_data ? "True":"False" );
...
static void
on_update_sensitivities( CactMainWindow *window, gpointer user_data )
{
	static const gchar *thisfn = "cact_main_menubar_on_update_sensitivities";
	...
	mis->updater = cact_application_get_updater( application );
	mis->is_level_zero_writable = na_pivot_is_level_zero_writable( NA_PIVOT( mis->updater ));

	mis->has_writable_providers = cact_window_has_writable_providers( CACT_WINDOW( window ));
	g_debug( "%s: has_writable_providers=%s", thisfn, mis->has_writable_providers ? "True":"False" );

	mis->selected_items = cact_iactions_list_bis_get_selected_items( CACT_IACTIONS_LIST( window ));
	...
	cact_main_menubar_file_on_update_sensitivities( window, user_data, mis );
	cact_main_menubar_edit_on_update_sensitivities( window, user_data, mis );
	cact_main_menubar_view_on_update_sensitivities( window, user_data, mis );
	cact_main_menubar_tools_on_update_sensitivities( window, user_data, mis );
	cact_main_menubar_maintainer_on_update_sensitivities( window, user_data, mis );
	cact_main_menubar_help_on_update_sensitivities( window, user_data, mis );
 ...
 */
void
cact_main_menubar_enable_item( CactMainWindow *window, const gchar *name, gboolean enabled )
{
...
{
	static const gchar *thisfn = "cact_main_menubar_on_delete_event";

	g_debug( "%s: toplevel=%p, event=%p, window=%p",
			thisfn, ( void * ) toplevel, ( void * ) event, ( void * ) window );

	cact_main_menubar_file_on_quit( NULL, window );
 ...
 * as data to the window ; it is so triggered on CactMainWindow::finalize()
 */
static void
on_destroy_callback( gpointer data )
{
	static const gchar *thisfn = "cact_main_menubar_on_destroy_callback";
 ...
 */
static void
on_menu_item_selected( GtkMenuItem *proxy, CactMainWindow *window )
{
	GtkAction *action;
	gchar *tooltip;

	/*g_debug( "cact_main_menubar_on_menu_item_selected: proxy=%p (%s), window=%p (%s)",
	...
	if( tooltip ){
		cact_main_statusbar_display_status( window, MENUBAR_PROP_STATUS_CONTEXT, tooltip );
	}
...
static void
on_menu_item_deselected( GtkMenuItem *proxy, CactMainWindow *window )
{
	cact_main_statusbar_hide_status( window, MENUBAR_PROP_STATUS_CONTEXT );
}

static void
on_popup_selection_done(GtkMenuShell *menushell, CactMainWindow *window )
{
	static const gchar *thisfn = "cact_main_menubar_on_popup_selection_done";
	MenubarIndicatorsStruct *mis;
...
static void
on_proxy_connect( GtkActionGroup *action_group, GtkAction *action, GtkWidget *proxy, CactMainWindow *window )
{
	static const gchar *thisfn = "cact_main_menubar_on_proxy_connect";
...
static void
on_proxy_disconnect( GtkActionGroup *action_group, GtkAction *action, GtkWidget *proxy, CactMainWindow *window )
{
