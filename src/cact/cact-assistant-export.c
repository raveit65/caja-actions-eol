...
/* private class data
 */
struct CactAssistantExportClassPrivate {
	void *empty;						/* so that gcc -pedantic is happy */
};

/* private instance data
 */
struct CactAssistantExportPrivate {
...
static CactAssistantExport *assist_new( BaseWindow *parent );
...
static void            on_initial_load_dialog( CactAssistantExport *dialog, gpointer user_data );
static void            on_runtime_init_dialog( CactAssistantExport *dialog, gpointer user_data );
static void            on_all_widgets_showed( CactAssistantExport *dialog );

static void            assist_initial_load_intro( CactAssistantExport *window, GtkAssistant *assistant );
static void            assist_runtime_init_intro( CactAssistantExport *window, GtkAssistant *assistant );

static void            assist_initial_load_actions_list( CactAssistantExport *window, GtkAssistant *assistant );
static void            assist_runtime_init_actions_list( CactAssistantExport *window, GtkAssistant *assistant );
static void            on_iactions_list_selection_changed( CactIActionsList *instance, GSList *selected_items );
static gchar          *on_iactions_list_get_treeview_name( CactIActionsList *instance );

static void            assist_initial_load_target_folder( CactAssistantExport *window, GtkAssistant *assistant );
static void            assist_runtime_init_target_folder( CactAssistantExport *window, GtkAssistant *assistant );
static GtkFileChooser *get_folder_chooser( CactAssistantExport *window );
static void            on_folder_selection_changed( GtkFileChooser *chooser, gpointer user_data );

static void            assist_initial_load_format( CactAssistantExport *window, GtkAssistant *assistant );
static void            assist_runtime_init_format( CactAssistantExport *window, GtkAssistant *assistant );
static NAExportFormat *get_export_format( CactAssistantExport *window );

static void            assist_initial_load_confirm( CactAssistantExport *window, GtkAssistant *assistant );
static void            assist_runtime_init_confirm( CactAssistantExport *window, GtkAssistant *assistant );

static void            assist_initial_load_exportdone( CactAssistantExport *window, GtkAssistant *assistant );
static void            assist_runtime_init_exportdone( CactAssistantExport *window, GtkAssistant *assistant );
...
register_type( void )
{
	...
	type = g_type_register_static( BASE_ASSISTANT_TYPE, "CactAssistantExport", &info, 0 );
...
{
	static const gchar *thisfn = "cact_assistant_export_iactions_list_iface_init";
...
instance_init( GTypeInstance *instance, gpointer klass )
{
	...
	g_assert( CACT_IS_ASSISTANT_EXPORT( instance ));
...
static CactAssistantExport *
assist_new( BaseWindow *parent )
{
	return( g_object_new( CACT_ASSISTANT_EXPORT_TYPE, BASE_WINDOW_PROP_PARENT, parent, NULL ));
}
...
static void
on_initial_load_dialog( CactAssistantExport *dialog, gpointer user_data )
{
	static const gchar *thisfn = "cact_assistant_export_on_initial_load_dialog";
	...
	g_return_if_fail( CACT_IS_ASSISTANT_EXPORT( dialog ));
...
static void
on_runtime_init_dialog( CactAssistantExport *dialog, gpointer user_data )
{
	static const gchar *thisfn = "cact_assistant_export_on_runtime_init_dialog";
	GtkAssistant *assistant;

	g_debug( "%s: dialog=%p, user_data=%p", thisfn, ( void * ) dialog, ( void * ) user_data );
	g_assert( CACT_IS_ASSISTANT_EXPORT( dialog ));
...
static void
on_all_widgets_showed( CactAssistantExport *dialog )
{
	static const gchar *thisfn = "cact_assistant_export_on_all_widgets_showed";

	g_debug( "%s: dialog=%p", thisfn, ( void * ) dialog );
	g_return_if_fail( CACT_IS_ASSISTANT_EXPORT( dialog ));

	cact_iactions_list_bis_select_first_row( CACT_IACTIONS_LIST( dialog ));
}

static void
assist_initial_load_intro( CactAssistantExport *window, GtkAssistant *assistant )
{
}

static void
assist_runtime_init_intro( CactAssistantExport *window, GtkAssistant *assistant )
{
	static const gchar *thisfn = "cact_assistant_export_runtime_init_intro";
	GtkWidget *content;
...
static void
assist_initial_load_actions_list( CactAssistantExport *window, GtkAssistant *assistant )
{
	g_assert( CACT_IS_IACTIONS_LIST( window ));

	cact_iactions_list_set_management_mode( CACT_IACTIONS_LIST( window ), IACTIONS_LIST_MANAGEMENT_MODE_EXPORT );
	cact_iactions_list_initial_load_toplevel( CACT_IACTIONS_LIST( window ));
}

static void
assist_runtime_init_actions_list( CactAssistantExport *window, GtkAssistant *assistant )
{
	BaseWindow *parent;
	GList *tree;
	GtkWidget *content;
	...
	g_assert( CACT_IS_MAIN_WINDOW( parent ));
	g_assert( CACT_IS_IACTIONS_LIST( parent ));
	tree = cact_iactions_list_bis_get_items( CACT_IACTIONS_LIST( parent ));
...
{
	static const gchar *thisfn = "cact_assistant_export_on_actions_list_selection_changed";
...
static gchar *
on_iactions_list_get_treeview_name( CactIActionsList *instance )
{
	gchar *name = NULL;
...
static void
assist_initial_load_target_folder( CactAssistantExport *window, GtkAssistant *assistant )
{
...
static void
assist_runtime_init_target_folder( CactAssistantExport *window, GtkAssistant *assistant )
{
...
get_folder_chooser( CactAssistantExport *window )
{
...
{
	static const gchar *thisfn = "cact_assistant_export_on_folder_selection_changed";
	GtkAssistant *assistant;
	gint pos;
	gchar *uri;
	gboolean enabled;
	...
	g_assert( CACT_IS_ASSISTANT_EXPORT( user_data ));
	assist = CACT_ASSISTANT_EXPORT( user_data );
	...
	if( pos == ASSIST_PAGE_FOLDER_SELECTION ){
		...
		if( enabled ){
			...
			cact_iprefs_write_string( BASE_WINDOW( assist ), IPREFS_EXPORT_ITEMS_FOLDER_URI, uri );
		}
...
static void
assist_initial_load_format( CactAssistantExport *window, GtkAssistant *assistant )
{
	CactApplication *application;
	NAUpdater *updater;
	GtkWidget *container;
	...
	cact_export_format_init_display( NA_PIVOT( updater ), container, EXPORT_FORMAT_DISPLAY_ASSISTANT );
}

static void
assist_runtime_init_format( CactAssistantExport *window, GtkAssistant *assistant )
{
	GtkWidget *content;
	GtkWidget *container;
	GQuark format;

	format = cact_iprefs_get_export_format( BASE_WINDOW( window ), IPREFS_EXPORT_FORMAT );
	container = base_window_get_widget( BASE_WINDOW( window ), "AssistantExportFormatVBox" );
	cact_export_format_select( container, format );
...
get_export_format( CactAssistantExport *window )
{
	GtkWidget *container;
...
static void
assist_initial_load_confirm( CactAssistantExport *window, GtkAssistant *assistant )
{
}

static void
assist_runtime_init_confirm( CactAssistantExport *window, GtkAssistant *assistant )
{
}

static void
assist_initial_load_exportdone( CactAssistantExport *window, GtkAssistant *assistant )
{
}

static void
assist_runtime_init_exportdone( CactAssistantExport *window, GtkAssistant *assistant )
{
}
...
assist_prepare_confirm( CactAssistantExport *window, GtkAssistant *assistant, GtkWidget *page )
{
	...
	cact_iprefs_set_export_format( BASE_WINDOW( window ), IPREFS_EXPORT_FORMAT, na_export_format_get_quark( format ));
...
assistant_apply( BaseAssistant *wnd, GtkAssistant *assistant )
{
	...
	g_assert( CACT_IS_ASSISTANT_EXPORT( wnd ));
	window = CACT_ASSISTANT_EXPORT( wnd );

	application = CACT_APPLICATION( base_window_get_application( BASE_WINDOW( window )));
	updater = cact_application_get_updater( application );
	actions = cact_iactions_list_bis_get_selected_items( CACT_IACTIONS_LIST( window ));
	...
	for( ia = actions ; ia ; ia = ia->next ){
		...
		str->format = cact_iprefs_get_export_format( BASE_WINDOW( wnd ), IPREFS_EXPORT_FORMAT );

		if( str->format == IPREFS_EXPORT_FORMAT_ASK ){
			str->format = cact_export_ask_user( BASE_WINDOW( wnd ), str->item );
